...
contract MultilateralClearing is EIP712, ReentrancyGuard {
    bytes32 private constant PATH_TYPEHASH = keccak256("PathOrder(uint256 incomingObligationId,uint256 outgoingObligationId,uint256 quantity,uint64 expiresAt,bytes32 salt)");
    bytes32 private constant CONFIRM_TYPEHASH = keccak256("SettlementConfirmation(bytes32 settlementId,bytes32 fulfillmentEvidenceHash,uint64 deadline)");

    enum InstructionStatus { NONE, PENDING, SETTLED, EXPIRED }
    ...
    event SettlementInstructionCreated(bytes32 indexed settlementId, uint256 indexed payerPassportId, uint256 indexed receiverPassportId, uint256 intermediaryPassportId, uint256 incomingObligationId, uint256 outgoingObligationId, uint256 quantity, uint64 dueDate, uint64 expiresAt, bytes32 consentProofHash);
    event SettlementInstructionStatusChanged(bytes32 indexed settlementId, InstructionStatus status, bytes32 fulfillmentEvidenceHash);
    ...
    function createPathInstruction(PathOrder calldata order, bytes calldata payerSig, bytes calldata intermediarySig, bytes calldata receiverSig) external nonReentrant returns (bytes32 settlementId) {
        if (block.timestamp > order.expiresAt) revert Expired();
        if (order.quantity == 0) revert InvalidQuantity();

        IMultidimensionalObligation.ObligationView memory incoming = obligations.getObligation(order.incomingObligationId);
        IMultidimensionalObligation.ObligationView memory outgoing = obligations.getObligation(order.outgoingObligationId);
        ...
        if (payer == 0 || intermediary == 0 || receiver == 0 || outgoing.issuerPassportId != intermediary || payer == receiver) revert InvalidPath();
        if (!_compatible(incoming, outgoing)) revert IncompatibleObligations();
        if (order.quantity > obligations.availableQuantity(order.incomingObligationId) || order.quantity > obligations.availableQuantity(order.outgoingObligationId)) revert InvalidQuantity();
        ...
        emit SettlementInstructionCreated(settlementId, payer, receiver, intermediary, order.incomingObligationId, order.outgoingObligationId, order.quantity, dueDate, order.expiresAt, consentProofHash);
    }
    ...
    function confirmPathSettlement(bytes32 settlementId, bytes32 fulfillmentEvidenceHash, uint64 confirmationDeadline, bytes calldata intermediarySig, bytes calldata receiverSig) external nonReentrant {
        ...
        bytes32 digest = _hashTypedDataV4(keccak256(abi.encode(CONFIRM_TYPEHASH, settlementId, fulfillmentEvidenceHash, confirmationDeadline)));
    ...
    function getInstruction(bytes32 settlementId) external view returns (Instruction memory) { return _instructions[settlementId]; }

    function pathDigest(PathOrder calldata order) external view returns (bytes32) { return _pathDigest(order); }

    function confirmationDigest(bytes32 settlementId, bytes32 fulfillmentEvidenceHash, uint64 deadline) external view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(CONFIRM_TYPEHASH, settlementId, fulfillmentEvidenceHash, deadline)));
    }

    function _pathDigest(PathOrder calldata order) private view returns (bytes32) {
        return _hashTypedDataV4(keccak256(abi.encode(PATH_TYPEHASH, order.incomingObligationId, order.outgoingObligationId, order.quantity, order.expiresAt, order.salt)));
    }
